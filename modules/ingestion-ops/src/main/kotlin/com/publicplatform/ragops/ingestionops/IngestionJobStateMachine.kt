...
object IngestionJobStateMachine {
    fun transition(
        ...
        // 제품 DB가 authoritative state 라는 전제를 유지하려면 허용 전이를 여기서 단일하게 강제해야 한다.
    ...
    private fun requireAllowedStatus(
    ...
    private fun requireAllowedStage(
        nextStatus: IngestionJobStatus,
        nextStage: IngestionJobStage,
    ) {
        val allowedStages =
            when (nextStatus) {
                ...
                IngestionJobStatus.RUNNING ->
                    setOf(
                        IngestionJobStage.FETCH,
                        IngestionJobStage.EXTRACT,
                        IngestionJobStage.NORMALIZE,
                ...
                IngestionJobStatus.FAILED,
                IngestionJobStatus.CANCELLED,
                -> setOf(IngestionJobStage.COMPLETE)
            }
    ...
    private fun resolveStartedAt(
        ...
        if (nextStatus == IngestionJobStatus.RUNNING) {
