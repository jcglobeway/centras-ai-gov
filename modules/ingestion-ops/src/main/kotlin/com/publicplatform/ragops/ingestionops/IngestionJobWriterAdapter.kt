...
open class IngestionJobWriterAdapter(
    ...
    override fun requestJob(command: RequestIngestionJobCommand): IngestionJobSummary {
        ...
        val entity = IngestionJobEntity(
            ...
            jobType = command.jobType.name.lowercase(),
            jobStatus = IngestionJobStatus.QUEUED.name.lowercase(),
            jobStage = IngestionJobStage.FETCH.name.lowercase(),
        ...
        val updatedSource = CrawlSourceEntity(
            ...
            scheduleExpr = source.scheduleExpr,
    ...
    override fun transitionJob(command: TransitionIngestionJobCommand): IngestionJobSummary {
        ...
        val updatedEntity = IngestionJobEntity(
            ...
            serviceId = currentEntity.serviceId,
            crawlSourceId = currentEntity.crawlSourceId,
            documentId = currentEntity.documentId,
            documentVersionId = currentEntity.documentVersionId,
            jobType = currentEntity.jobType,
            jobStatus = transitionedJob.status.name.lowercase(),
            jobStage = transitionedJob.stage.name.lowercase(),
            triggerType = currentEntity.triggerType,
            runnerType = currentEntity.runnerType,
            attemptCount = transitionedJob.attemptCount,
            errorCode = transitionedJob.errorCode,
            requestedAt = currentEntity.requestedAt,
        ...
        // Update crawl source status based on job result
    ...
    private fun updateSourceAfterTransition(job: IngestionJobSummary) {
        ...
        val updatedStatus = when (job.status) {
            ...
            -> CrawlSourceStatus.ACTIVE.name.lowercase()
