...
export function Sidebar() {
  ...
  return (
    <aside className="w-60 shrink-0 bg-bg-base flex flex-col h-screen sticky top-0">
      {/* brand */}
      <div className="px-6 py-5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-text-primary text-xs font-semibold tracking-tight">Centras AI Gov</p>
            <p className="text-text-muted text-[11px] font-mono uppercase tracking-widest mt-0.5">{active.label}</p>
          </div>
        </div>
      </div>
        ...
        {active.nav.map((entry, idx) => {
          if ('section' in entry) {
            return (
              <p
                key={`section-${idx}`}
                className="font-mono text-[10px] uppercase tracking-[0.15em] text-text-muted px-4 pt-4 pb-1"
              >
                {entry.section}
              </p>
            );
          ...
          return (
            <Link
              ...
              href={item.href}
              className={clsx(
                "flex items-center gap-2.5 px-3 py-2 text-xs transition-colors duration-150 border-l-[3px]",
                isActive
            ...
            >
              <span className={clsx("material-symbols-outlined text-[18px] leading-none", isActive && "text-accent")}>
                {item.icon}
              </span>
      ...
      {/* System Health LED */}
      <div className="mx-3 mb-4 p-3 bg-bg-elevated rounded-lg">
        <p className="font-mono text-[10px] uppercase tracking-widest text-text-muted mb-2">System Health</p>
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-success" style={{ boxShadow: '0 0 6px var(--success)' }} />
          <span className="font-mono text-[11px] text-text-secondary">Operational</span>
        </div>
      </div>
    </aside>
  );
